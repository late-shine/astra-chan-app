...
The working method was deliberately practical: I described the behavior and visual target, supplied screenshots or the relevant files, planned larger changes in phases, and kept the untouched app separate from dated working copies. Codex and the other AI tools handled implementation and code inspection; I tested the result locally and in Edge, Chrome, Firebase, and Vercel, then reported what actually happened. That feedback loop—not a single perfect prompt—is how the app became reliable enough to use.
...
<p align="center">
  <img src="docs/reading-room.png" alt="Reading Room — N5 passages with guided help, optional timer, and Astra-chan companion" width="100%" />
</p>
...
- **Authenticated answer attribution** — submitted answers derive the host/guest role and display name from the Firebase UID and room state instead of trusting browser-supplied role data
...
When using the Firebase CLI, publish database rules with:
...
V1 protects authentication, room membership IDs, invite ownership, profiles, and private progress records. Answer attribution is derived from `auth.uid` rather than a client-provided role. The quiz result and timing are still reported by the client, and room updates use client transactions, so a determined user with a modified browser can cheat. That boundary is acceptable for friendly matches; a tournament-grade anti-cheat system would need server-authoritative scoring.
...
- [x] Auth-derived multiplayer answer attribution
